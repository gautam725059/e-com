...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (<main className="max-w-4xl mx-auto px-4 py-16 text-center"> <div className="bg-white rounded-xl shadow p-10"> <h1 className="text-2xl font-semibold mb-4">
      Your cart is empty </h1>

      <Link
        href="/products"
        className="inline-block bg-navy-700 text-white px-6 py-3 rounded-lg"
      >
        Browse Products
      </Link>
    </div>
...
return ( <main className="max-w-6xl mx-auto px-4 py-10"> <h1 className="text-3xl font-bold mb-8">
Your Cart </h1>
      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            <button
              className="text-red-600 text-sm md:ml-4"
              onClick={() =>
                removeItem(item.id)
              }
            >
    ...
    {products.slice(0, 4).map((product: any) => (
      ...
      >
        ...
        <div className="p-4">
          ...
          <p className="text-navy-700 font-bold mt-2">
            ₹{product.price}
          </p>
        </div>
      </Link>
